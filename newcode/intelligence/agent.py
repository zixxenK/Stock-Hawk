...
class PatternMemory:
    ...
    def find_similar(
        ...
        for stored_state, outcome in zip(self._states, self._outcomes):
            stored_norm = np.linalg.norm(stored_state)
            if stored_norm == 0.0:
                continue
            similarity = float(np.dot(state, stored_state) / (current_norm * stored_norm))
    ...
    def historical_win_rate(self, state: np.ndarray, top_k: int = 20) -> float | None:
        ...
        if not similar:
            return None
        wins = sum(1 for s in similar if s.get("trade_return", 0.0) > PROFIT_THRESHOLD)
...
class FlippyAgent:
    ...
    def __init__(
        ...
        self._weights: np.ndarray = np.zeros((N_ACTIONS, EXPANDED_DIM), dtype=np.float64)
    ...
    def _train_step(self) -> None:
        ...
        for experience in batch:
            state_phi = expand_features(experience.state).astype(np.float64)
            q_values = self._q_values(experience.state)
            td_target = experience.reward
            if not experience.done and experience.next_state is not None:
                td_target += self.discount * float(np.max(self._q_values(experience.next_state)))

            td_error = td_target - float(q_values[experience.action])
            self._weights[experience.action] += self.learning_rate * td_error * state_phi
            total_loss += float(abs(td_error))

        self._last_loss = total_loss / max(1, len(batch))
        self._step += 1
        self.epsilon = max(self.epsilon_min, self.epsilon * self.epsilon_decay)

    def _save_weights(self) -> None:
        try:
            path = WEIGHTS_DIR / "agent_weights.npy"
            np.save(path, self._weights)
        except Exception as exc:
            logger.debug("Failed to save agent weights: %s", exc)
